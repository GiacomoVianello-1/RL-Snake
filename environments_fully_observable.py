...
class BaseEnvironment:
    ...
    def __init__(self, n_boards, board_size):
        self.WIN_REWARD = 10.            # originally = 1.0,  but good = 10
        self.FRUIT_REWARD = 0.5          # originally = 0.5,  leave as it is
        self.STEP_REWARD = -0.01         # originally = 0.0,  but good = -0.01
        self.ATE_HIMSELF_REWARD = -.5    # originally = -0.2, but good = -1.0
        self.HIT_WALL_REWARD = -.1       # originally = -0.1, leave it as it is
    ...
    def move(self, actions):
        ...
        # Replacement for tf.reshape(tf.convert_to_tensor(...))
        return torch.tensor(rewards, dtype=torch.float32).view(-1, 1)

    def to_state(self):
        # Replacement for K.utils.to_categorical(self.boards)[..., 1:]
        # Creates one-hot encoding for classes 0..4 and removes class 0 (WALL)
        return np.eye(5, dtype=np.float32)[self.boards.astype(int)][..., 1:]
